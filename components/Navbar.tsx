        ...
        <div className="hidden lg:flex space-x-6">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `relative py-2 text-xs xl:text-sm uppercase tracking-widest transition-colors ${
                  isActive ? "text-white font-medium" : "text-cnxh-light/70"
                }`
              }
            ...
            >
              {({ isActive }) => (
                <>
                  {item.label}
                  <span
                    className={`nav-line absolute bottom-0 left-0 h-[1px] bg-cnxh-red transition-all duration-300 ${
                      isActive ? "w-full" : "w-0"
                    }`}
                  ></span>
                </>
